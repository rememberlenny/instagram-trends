# How to provision a new environment for instagram-trends
...
* Enter domains to secure (for example): `instagram-trends.herokuapp.com`
* Enter webroot path: `/home/deployer/apps/instagram_trends/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/instagram-trends.herokuapp.com/fullchain.pem /etc/ssl/instagram_trends.crt
ln -s /etc/letsencrypt/live/instagram-trends.herokuapp.com/privkey.pem /etc/ssl/instagram_trends.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/instagram_trends/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
48 0,12 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/instagram_trends/shared/public/
```
